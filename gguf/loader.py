...
def gguf_sd_loader(path, handle_prefix="model.diffusion_model.", is_text_model=False):
    ...
    if arch_str in [None, "pig", "cow"]:
        ...
        # from .tools.convert import detect_arch # REMOVED: Dependency on tool we didn't port. Assuming explicitly supported archs for now or basic fallback.
        # Minimal compatible fallback logic if possible, or error out
        # try:
        #     arch_str = detect_arch(set(val[0] for val in tensors)).arch
        # except Exception as e:
        #     raise ValueError(f"This model is not currently supported - ({e})")
        logging.warning("Auto-detection for sd.cpp/pig/cow models is limited in this port.")
