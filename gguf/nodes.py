...
# Initialize GGUF path if not present (though h4_loaders handles filtering mostly)
if "unet_gguf" not in folder_paths.folder_names_and_paths:
    folder_paths.folder_names_and_paths["unet_gguf"] = (folder_paths.folder_names_and_paths.get("diffusion_models", ([], {}))[0], {".gguf"})
...
class UnetLoaderGGUF:
    # NOTE: NOT A COMFY NODE. Internal helper class.
    ...
    def load_unet(self, unet_name, dequant_dtype=None, patch_dtype=None, patch_on_device=None):
        ...
        # init model
        # Check standard paths first, then try full path if absolute
        unet_path = folder_paths.get_full_path("unet", unet_name)
        if not unet_path:
             unet_path = folder_paths.get_full_path("diffusion_models", unet_name)
        if not unet_path and os.path.exists(unet_name):
            unet_path = unet_name
            
        if not unet_path:
             raise FileNotFoundError(f"Could not find model file: {unet_name}")
...
# NOTE: Not exposing NODE_CLASS_MAPPINGS to avoid conflict.
